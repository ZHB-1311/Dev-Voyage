...
## 14.11 新手加餐：部署不是把代码丢到服务器就完事

部署的本质是：让你的程序在另一台长期运行的电脑上稳定提供服务。

本地开发时，你可以手动开终端、手动重启、手动看报错。但线上服务需要更稳定的机制。

### 14.11.1 本地运行和线上运行有什么区别？

| 对比 | 本地开发 | 线上部署 |
| --- | --- | --- |
| 运行位置 | 你的电脑 | 云服务器 |
| 启动方式 | 手动运行命令 | systemd、Docker、平台自动启动 |
| 访问方式 | localhost | 域名或公网 IP |
| 错误查看 | 终端窗口 | 日志系统 |
| 稳定性要求 | 断了可以重开 | 应该自动恢复 |

### 14.11.2 Nginx 像什么？

Nginx 可以理解成网站门口的保安和前台：

```text
用户访问域名 -> Nginx 接待 -> 转发给前端或后端服务
```
...
### 14.11.3 Docker 解决什么问题？

Docker 最常见的价值是解决："为什么你电脑能跑，我服务器不能跑？"

容器把运行环境一起打包：

```text
代码 + 依赖 + 运行命令 + 系统环境的一部分 = 镜像
```

然后服务器只要能运行 Docker，就能用同样方式启动项目。
...
课程结束后，不要急着追最新框架。建议按这个顺序巩固：

```text
先把博客项目改成自己的作品
再独立做一个小项目
再学习测试、TypeScript、PostgreSQL
最后再碰微服务、K8s 这类复杂主题
```

技术路线不是越高级越好，而是越能解决你当前的问题越好。
