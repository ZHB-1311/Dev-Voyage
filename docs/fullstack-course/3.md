...
这一章你已经跑通了第一个全栈页面，但如果只是照着敲完代码，很容易出现一种错觉："我好像会了"。真正重要的是理解每个东西在系统里扮演什么角色。

::: info 一句话理解全栈请求链路
用户看到的是前端页面，前端页面向后端要数据，后端把数据整理成 JSON 返回给前端，前端再把 JSON 渲染成用户看得懂的文字和界面。
:::
...
| --- | --- | --- |
| 用户 | 浏览器访问页面的人 | 点奶茶的人 |
| 前端 | Nuxt 页面 | 店员，负责展示菜单和接待 |
| 后端 | FastAPI API | 后厨，负责真正制作内容 |
| JSON | API 返回的数据格式 | 小票，上面写清楚要什么 |
| 端口 | `3000`、`8000` | 不同窗口号码 |

### 3.7.1 为什么前端和后端要分两个端口？

开发时我们通常会同时启动两个服务：

```text
前端 Nuxt:   http://localhost:3000
后端 FastAPI: http://localhost:8000
```

这不是因为网站最终一定要长这样，而是因为开发阶段前端和后端由不同工具运行：Nuxt 负责热更新页面，FastAPI 负责运行 Python API。两个程序都要占用一个"门牌号"，这个门牌号就是端口。

::: tip 端口记忆法
`localhost` 表示"我自己的电脑"，端口表示"我电脑上的第几个服务窗口"。

所以 `localhost:3000` 和 `localhost:8000` 不是两台电脑，而是同一台电脑上的两个服务。
:::
...
虽然它们都在你的电脑上，但端口不同，浏览器就会认为它们是两个"不同来源"。为了保护用户，浏览器默认不允许一个来源随便读取另一个来源的数据。

这就是为什么我们要在 FastAPI 里写：
...
```text
后端告诉浏览器：localhost:3000 是我认识的前端，可以来拿数据。
```
...
1. 前端服务有没有启动？浏览器能不能打开 `http://localhost:3000`？
2. 后端服务有没有启动？浏览器能不能打开 `http://localhost:8000/api/greeting`？
3. 前端请求地址有没有写错？是不是 `http://localhost:8000/api/greeting`？
4. 后端有没有配置 CORS？`allow_origins` 里有没有前端地址？
5. 终端有没有报错？报错第一行和最后一行通常最有用。
:::
...
@app.get("/api/time")
async def get_time():
    return {"text": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
```
...
```vue
<script setup>
const { data: timeData } = await useFetch('http://localhost:8000/api/time')
</script>
```
