...
SSH（Secure Shell）让你用命令行连接到远程服务器。课程实验用的服务器、租的云服务器、宿舍的旧电脑——只要开了 SSH，就能在本地终端里操作它们。

**基本用法：**
```bash
ssh user@服务器IP地址
```
输入密码后，你就在那台远程电脑的终端里了。敲的所有命令都在远程执行。

**避免每次输密码（SSH 密钥登录）：**
```bash
ssh-keygen -t ed25519            # 生成一对密钥
ssh-copy-id user@服务器IP         # 把公钥传到服务器
```
之后再登录就不需要密码了。

**搭配 tmux：**
SSH 连接中断后正在运行的任务也会被终止。tmux 可以保持会话在后台运行：
```bash
tmux new -s mytask                # 创建会话
# 运行长任务...
# Ctrl+B 然后按 D 断开（任务继续）
tmux attach -t mytask             # 重新连回
```

## 📜 脚本化：把命令串起来

你已经学会了 `ls`、`cd`、`grep`、`|` 这些命令。如果有一串操作要重复执行，每次手动敲一遍太傻了。
...
mkdir -p backup
cp *.jpg backup/
echo "备份完成，共 $(ls backup/*.jpg | wc -l) 个文件"
```
...
你不用成为 shell 编程高手。把常用的重复操作写成脚本存起来，需要时执行，这就是命令行效率的终极体现。
